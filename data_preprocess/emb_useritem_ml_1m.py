...
if __name__ == "__main__":
    ...
    parser.add_argument("--embed_dir", type=str, default="../data/ml-1m/PLM_data")
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--gpu_id", type=int, default=0)
    parser.add_argument("--model_name", type=str, default="vicuna")
    args = parser.parse_args()
    args.model_path = "../models/vicuna/"

    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_id)
    # Reset default repetition penalty for T5 models.
    assert args.pooling in ["average", "last"], "Pooling type error"

    data_dir = "../data/ml-1m/proc_data/data/intermediate_data"
    user_path = "../data/ml-1m/raw_data/users.dat"
    item_path = "../data/ml-1m/raw_data/movies.dat"
    rating_path = "../data/ml-1m/raw_data/ratings.dat"
    ...
    for line in open(user_path, "r").readlines():
        ...
        user_id = int(user_id)
        user_dict[user_id] = (
            f'This is a user, gender is {user_feature[gender]}, age is {user_feature["age"][age]}, job is {user_feature["occupation"][job]}.'
        )
    print(user_dict[250])
    ...
    meta_data = json.load(open(os.path.join(data_dir, "ctr-meta.json"), "r"))
    ...
    user_index = np.zeros([6041, 5120])
    # item_embedding search table
    item_index = np.zeros([3706, 5120])
